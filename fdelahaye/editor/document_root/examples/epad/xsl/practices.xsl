...
-->
  ...
  <xsl:param name="internal-nav" select="practices/h"/>

  <!-- Main page structure -->
  <xsl:template match="/">
    <html>
      <head>
        <title><xsl:value-of select="practices/h"/></title>
      </head>
      <body>
  ...
  <xsl:template match="section|practice">
    <div class="section">
      <xsl:apply-templates />
    </div>
  </xsl:template>
  ...
  <xsl:template match="section[@view='list']|practice[@view='list']">
    <div>
      ...
      <xsl:apply-templates select="*[local-name() != 'section' and local-name() != 'practice']"/>
      <div class="content">
        <ul>
          <xsl:apply-templates select="section|practice" mode="list-item"/>
        </ul>
      </div>
  ...
  <xsl:template match="*[not(@view)]/section|practice/h" mode="number">
    <xsl:attribute name="class">
    ...
    <xsl:if test="/practices/@section-numbering='true'">
      <span class="h-numbering">
        <xsl:number count="practice|section|practice" from="/practices" level="multiple" format="1"/>.
      </span>
    </xsl:if>
  </xsl:template>

  <!-- headers -->
  <xsl:template match="practices/h">
    <h1>
      <xsl:apply-templates select="@*"/>
      <xsl:apply-templates />
    </h1>
  ...
  <xsl:template match="section|practice/h">
    <h2>
      <xsl:apply-templates select="@*"/>
  ...
  <xsl:template match="section/h|ol/h|ul/h">
    <h3>
      <xsl:apply-templates select="."  mode="number"/>
      <xsl:apply-templates select="@*"/>
      <xsl:apply-templates />
  ...
  <xsl:template match="section/h|hli/h">
    <h4>
      <xsl:apply-templates select="."  mode="number"/>
      <xsl:apply-templates select="@*"/>
      <xsl:apply-templates />
