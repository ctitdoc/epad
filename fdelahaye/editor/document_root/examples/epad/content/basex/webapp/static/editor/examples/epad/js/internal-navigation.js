...
function scanDocument(evt)
{
  ...
  //var specialsections = xmldoc.selectNodes("//section[@view = 'tabs' or @view = 'accordion']|dataset_definition[@view = 'tabs' or @view = 'accordion']|data_definition[@view = 'tabs' or @view = 'accordion']|data_constraints_definition[@view = 'tabs' or @view = 'accordion']|property_definition[@view = 'tabs' or @view = 'accordion']|property_constraints_definition[@view = 'tabs' or @view = 'accordion']/section[1]|dataset_definition[1]|data_definition[1]|data_constraints_definition[1]|property_definition[1]|property_constraints_definition[1]");
  var specialsections = xmldoc.selectNodes("//*[local-name()='section' or @behavior='section']");
...
function afterNodeValueModified(evt)
{
  ...
  if (node)
  {
    ...
    {
      var subsect = node.getOwnerElement().selectSingleNode("section|dataset_definition|data_definition|data_constraints_definition|property_definition|property_constraints_definition");
      if(subsect)
...
function updateInternalNavigation(evt)
{
  ...
  if (currentNode.getParentNode() && currentNode.getParentNode().getLocalName() != "article")
    while (currentNode && currentNode.getLocalName() != "section"
    && currentNode.getLocalName() != "dataset_definition"
    && currentNode.getLocalName() != "data_definition"
    && currentNode.getLocalName() != "data_constraints_definition"
    && currentNode.getLocalName() != "property_definition"
    && currentNode.getLocalName() != "property_constraints_definition"
    && currentNode.getParentNode() != document)
      currentNode = currentNode.getParentNode();
